...
bool CollisionTester::collisionTest(GLfloat x, GLfloat y, GLfloat z){
	for(int i = 0; (i < (int)staticBoxes.size()); i++){
		if(collision(staticBoxes.at(i),x,y,z)){
			return true;
		}
	}
	return false;
...
bool CollisionTester::collision(BoundingBox* b, GLfloat x, GLfloat y, GLfloat z){
